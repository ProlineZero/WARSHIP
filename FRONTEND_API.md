...
## ⚠️ ДОРАБОТКИ API — режим `peer` (ходы через Centrifugo)

> **Инструкция для ИИ-агента:** этот блок — **только новые изменения** поверх разделов **0–14** ниже.
> Базовый протокол (`server`: HTTP `make_shot`, полная доска в `GET board`, §7.4, §8.4) **уже описан** — **не переписывай** его.
> Реализуй ветку **peer** **дополнительно**, если в `GET /status/` приходит `"play_mode": "peer"`.

**Дата доработки:** 2026-05.

### Суть

| | `play_mode: "server"` (как в §0–14) | `play_mode: "peer"` (**эта доработка**) |
|---|-------------------------------------|----------------------------------------|
| Выстрелы | `POST make_shot/` → бэкенд → WS `shot_result` | Клиент публикует в WS `game_{id}`, бэкенд **не** обрабатывает ход |
| Доска после старта | `GET board/` — корабли и выстрелы | `GET board/` — **пустые** массивы; состояние **только на клиенте** |
| Координаты кораблей в БД | Хранятся до конца игры | После `game_started` **удаляются** с сервера |
| Победитель | Через `make_shot` / admin approve | Публикация в канал `finish:{game_id}` → бэкенд пишет результат **1 раз** |

### Когда включается `peer`

- Тренировочный matchmaking ботов: `POST matchmaking/find/` с `is_training: true` (бот-JWT) → игра создаётся с `play_mode: "peer"`.
- Admin: `POST /api/admin/games/{id}/control/` с `"play_mode": "peer"`.
- Admin bot vs bot / admin vs bot (training) — по умолчанию `peer`.
- Рейтинговые human-игры (`is_training: false`) — по умолчанию **`server`**.

### D.1. Поле `play_mode` в статусе

Присутствует в `GET /status/` → `data`, в `active_game_found` → `data`, в WS `game_status` / `game_started` → `data`:
...
| Значение | Смысл |
|----------|--------|
| `server` | Классический режим (§7.4, §8.4) |
| `peer` | Ходы и победа по правилам **этого раздела** |

### D.2. HTTP — изменения и новые эндпоинты

#### Без изменений (только для peer)

- `POST place_ships/` — валидация расстановки **как раньше** (§7.3).
- После старта обоих игроков — WS `game_started` (в `data` есть `"play_mode": "peer"`).

#### `POST make_shot/` — **запрещён** в peer
...
#### `GET board/` — пустое состояние после старта

При `play_mode: "peer"` и статусе ≠ `waiting_ships`:
...
> UI хранит флот локально после успешного `place_ships` (ответ сервера не возвращает корабли повторно).

#### `POST finish/` — **НОВЫЙ** (fallback)

```http
POST /api/warship/game/{game_id}/finish/
```
...
Ответ `200` (только `play_mode: "peer"`):
...
Идемпотентно, если игра уже `finished`. Основной путь для ботов — WS `finish:{id}` (§D.4), не обязательно вызывать HTTP.

#### Блокировка следующей игры

Пока активная игра не в `finished` / `cancelled`, `POST matchmaking/find/` возвращает `active_game_found` (как §5.1). В `data` есть `"play_mode"`.

### D.3. Centrifugo — каналы в режиме peer

Дополнительно к §8.1:

| Канал | Подписка | Publish клиентом | Назначение |
|-------|----------|------------------|------------|
| `game_{game_id}` | ✓ участник | ✓ подписчик (`shot`, `shot_result`) | Ходы в реальном времени |
| `finish:{game_id}` | ✓ участник | ✓ (`game_finished`) | Запись победителя в БД через publish proxy |

**Безопасность (доработка subscribe proxy):**

- `user_{user_id}` — подписаться может **только** пользователь с `id == user_id`.
- `finish:{id}` — только участник peer-игры, игра не в `finished` / `cancelled`.

> Публикация в `game_{id}` **не** проходит через Django (высокая частота ходов). Публикация в `finish:{id}` — **один** HTTP-запрос proxy на партию.

### D.4. WS-протокол peer (канал `game_{game_id}`)

Клиент **публикует** (не только слушает):

#### `shot` — стреляющий (свой ход)
...
#### `shot_result` — защитник (владелец поля)

После приёма `shot` вычисляет попадание **локально** и публикует:
...
При уничтожении всех кораблей добавить:
...
**Правила хода (на клиенте):** как §4.3 — попадание → тот же `next_turn_id` == `shooter_id`; промах → `next_turn_id` == защитник.

#### События с сервера на `game_{id}` (без изменений)

`game_started`, `game_status`, `game_cancelled` — как §8.4.

#### `game_finished` на `game_{id}` — после записи в БД

Приходит **с сервера** после успешной публикации в `finish:{id}`:
...
```json
{
  ...
  "data": {
    "game_id": 42,
    "winner": { "id": 2, "username": "p2" },
    "finished_at": "2026-05-27T12:30:00+00:00"
  }
}
```

### D.5. Завершение игры (канал `finish:{game_id}`)

Победитель (или клиент, зафиксировавший победу) **публикует**:
...
Centrifugo → publish proxy → Django: `finish_game`, очистка координат в БД, WS `game_finished` на `game_{id}`.

**Альтернатива:** `POST /finish/` (§D.2).

### D.6. Admin — доработка

`POST /api/admin/games/{id}/control/` — добавлено поле:

```json
{ "play_mode": "peer" }
```

Допустимые значения: `server`, `peer`. Отображается в `GET /status/`, `GET /api/admin/games/{id}/`.

> В peer admin `GET board/` после старта тоже не содержит кораблей (данные сняты с сервера). Наблюдение ходов — через WS `game_{id}`.

### D.7. Порядок интеграции UI (только peer)

1. `GET /status/` → если `play_mode !== "peer"` — использовать §0–14.
2. После `place_ships` сохранить флот **в памяти клиента**.
3. Subscribe: `game_{id}` + **`finish:{id}`**.
4. Цикл: на свой ход → `publish shot`; на чужой `shot` → `publish shot_result`.
5. При победе → `publish` в `finish:{id}`; ждать WS `game_finished` на `game_{id}`.
6. Не вызывать `make_shot`. Не полагаться на `GET board` после старта.

### D.8. Чеклист для ИИ-агента (только доработки peer)

- [ ] Ветвление по `data.play_mode` после `status` / `game_found` / `active_game_found`
- [ ] Subscribe `finish:{game_id}` в дополнение к `game_{game_id}`
- [ ] Client publish: `shot` / `shot_result` на `game_{id}`
- [ ] Client publish: `game_finished` + `winner_id` на `finish:{id}`
- [ ] Локальное состояние флота и выстрелов (не `GET board` после старта)
- [ ] Не вызывать `POST make_shot` при `peer` (ожидать `409`)
- [ ] Опционально `POST finish/` как fallback
- [ ] Не начинать новый matchmaking, пока активная peer-игра не `finished`

### D.9. Сводка новых/изменённых точек API

| Метод | URL | Изменение |
|-------|-----|-----------|
| — | `data.play_mode` в status / WS | **НОВОЕ поле** |
| `POST` | `/api/warship/game/{id}/make_shot/` | **409** при `peer` |
| `GET` | `/api/warship/game/{id}/board/` | Пустые массивы при `peer` после старта |
| `POST` | `/api/warship/game/{id}/finish/` | **НОВЫЙ** эндпоинт |
| WS publish | `game_{id}` | `shot`, `shot_result` (**НОВОЕ**) |
| WS publish | `finish:{id}` | `game_finished` + `winner_id` (**НОВОЕ**) |
| `POST` | `/api/admin/games/{id}/control/` | поле `play_mode` (**НОВОЕ**) |

**Референс-реализация (peer):** `game_client/peer_battle.py`, `game_client/load_test/async_realtime.py`, `game_client/load_test/player.py`.
...
Игровой фронтенд
  ...
  └─ WebSocket  →  /connection/websocket        (Centrifugo: игра найдена, выстрел, конец игры)

Admin-панель
  ├─ HTTP REST  →  /api/admin/...               (группы, пользователи, статистика, управление играми)
  └─ WebSocket  →  /connection/websocket        (наблюдение за game_{id}, staff может подписаться на любую игру)
```

**Правило (режим `server`):** пользователь **инициирует действия через HTTP**, а **обновления UI получает через Centrifugo** (и при необходимости дублирует через HTTP polling `GET /status/` и `GET /board/`).

> **Доработка `peer`:** ходы идут через **WS publish** на `game_{id}`, не через HTTP — см. раздел **«ДОРАБОТКИ API — режим peer»** в начале файла.

На сервере nginx проксирует всё через **порт 80** — отдельные порты `:8000` / `:8001` снаружи не используются.
...
| Сервис | URL |
|--------|-----|
| REST API | `http://135.106.139.244/api` |
| Admin REST API | `http://135.106.139.244/api/admin` |
| Centrifugo WS | `ws://135.106.139.244/connection/websocket` |
| Healthcheck nginx | `http://135.106.139.244/health` |

> Для фронтенда удобно задать одну базу: `http://135.106.139.244` — все пути (`/api/...`, `/connection/...`) идут на тот же хост.
...
> Локально сервисы доступны напрямую по портам. На проде те же пути, но через nginx на `:80`.
...
4. **Экран игры** → подписаться на `game_{game_id}`, `POST place_ships`, цикл `GET board` + `POST make_shot` *(или ветка **peer** — см. доработки в начале файла)*
...
| `409` | `В режиме peer ходы выполняются через Centrifugo` + `play_mode` | **`play_mode: "peer"`** — см. **доработки в начале файла** |

### 7.5. Завершить игру (только `peer`) — **ДОРАБОТКА**

> Полное описание — в разделе **«ДОРАБОТКИ API — режим peer»** (§D.2, §D.5). В режиме `server` этот эндпоинт не используется.
...
| `finish:{game_id}` | Только при `play_mode: "peer"` | публикация `game_finished` → запись победителя в БД (**доработка**, см. начало файла) |

### 8.2. Безопасность каналов

- **Игрок** — subscribe proxy разрешает `game_{id}` только участникам; `user_{id}` — только своему `id` (**доработка**).
- **`finish:{id}`** — только участник peer-игры (**доработка**).
- **Admin (`is_staff=true`)** — может подписаться на **любой** `game_{id}` (read-only наблюдение).
- **Client publish** на `game_{id}` — только подписчикам канала; в `peer` ходы не проходят через Django (**доработка**).
...
Допустимые `play_mode` (**доработка**): `server`, `peer`.
...
| POST | `/api/warship/game/{id}/make_shot/` | ✓ | Выстрел (409 при `peer`) |
| POST | `/api/warship/game/{id}/finish/` | ✓ | **ДОРАБОТКА:** завершение peer-игры |
...
- [ ] **ДОРАБОТКА peer:** см. чеклист в начале файла (§D.8) — не смешивать с пунктами выше для `server`
- [ ] При unmount: `leave` + unsubscribe `game_{id}` (+ `finish:{id}` в peer)
...
- `game_client/realtime.py` — Centrifugo (режим `server`)
- `game_client/peer_battle.py`, `game_client/load_test/async_realtime.py` — **доработка peer**
